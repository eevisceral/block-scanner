  ...
  function getBlockData() {
    ...
    .on("data", async (blockHead) => {
      try {
        ...
        const blockMinusOne = (currentBlock - 1);
        const blockMinusThirty = (currentBlock - 30);

        // push data to array without duplicates
        if (numArray.includes(currentBlock) === false) numArray.push(currentBlock);

        // get transaction count
        const txCount = await web3.eth.getBlockTransactionCount(currentBlock);

        // create array of transactions in block
        const currentBlockData = await web3.eth.getBlock(currentBlock);
        const txArray = currentBlockData.transactions;

        if (allTxArray.includes(currentBlockData.transactions) === false)
        allTxArray.push(currentBlockData.transactions);

        allTxArrayLength = allTxArray.length;
        const allTxCount = (allTxArrayLength - 1);
    ...
    .on("data", async (txData) => {
      try {
        ...
        //console.log(data);
        console.log("Total Txs in Block: " + allTxCount);
        console.log("Scanning Tx #" + b + " / " + allTxCount);

         // get latest tx receipt from all tx array
         const txReceipt = await web3.eth.getTransactionReceipt(allTxArray[b]);
         var b = b++;
